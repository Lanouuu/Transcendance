	...
	try {
		...
		if (response.message === "Success") {
			const res1 = await fetch(`${gameRoute}/tournamentAlias`, {
				...
				body: JSON.stringify({ alias: aliasArray, mode: tournamentMode, tournament_id: response.tournament_id }) // Envoyer l'alias dans le body
			});
	...
	try {
		...
		if (response.message === "Success") {
			const res1 = await fetch(`${gameRoute}/tournamentAlias`, {
				...
				body: JSON.stringify({ alias: aliasArray[0], mode: "remote", tournament_id: response.tournament_id }) // Envoyer l'alias dans le body
			});
